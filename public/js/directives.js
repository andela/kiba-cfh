  ...
  .directive('dashboardTabs', () => {
    return {
      ...
      controller: ['$scope', function ($scope) {
        const panes = [];
        $scope.select = (pane) => {
          angular.forEach(panes, (currentPane) => {
      ...
      templateUrl: 'views/dashboard-tabs.html'
    };
  })
  .directive('leaderBoard', ['$http', $http => (
    {
      ...
      link: (scope, element, attribs, tabsController) => {
        scope.title = 'LeaderBoard';
        tabsController.addPane(scope);
        $http.get('api/leaderboard')
          .then((response) => {
            const gameWinners = response.data.result.map(record => record.gameWinner);
            const gameWinnerAggregates = gameWinners.reduce((aggregate, value) => {
              const index = aggregate.map(x => x.name).indexOf(value);
              if (index > -1) {
                aggregate[index].frequency += 1;
              } else {
                aggregate.push({
                  name: value,
                  frequency: 1
            ...
            // gameWinnerAggregates = gameWinnerAggregates.slice(0);
            gameWinnerAggregates.sort((a, b) => b.frequency - a.frequency);
            scope.leaderboard = gameWinnerAggregates;
          });
      },
      templateUrl: 'views/leaderboard.html',
    })]
  )
  .directive('gameHistory', ['$http', $http => (
    {
      restrict: 'AE',
      replace: true,
      require: '^^dashboardTabs',
      ...
      link: (scope, element, attribs, tabsController) => {
        scope.title = 'Game History';
        tabsController.addPane(scope);
        $http.get('/api/games/history')
          .then((response) => {
            scope.gameHistory = response.data.result;
          });
      },
  ...
  .directive('donations', ['$http', $http => (
    {
      ...
      require: '^^dashboardTabs',
      link: (scope, element, attribs, tabsController) => {
        scope.title = 'Donations';
        scope.user = user.name;
        tabsController.addPane(scope);
        $http.get('/api/donations')
          .then((response) => {
            scope.donations = response.data.result;
          });
      },
